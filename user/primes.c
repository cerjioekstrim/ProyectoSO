...
void
filtro(int pipe_izq[2])
{
  close(pipe_izq[WRITE]); //se cierra la escritura del pipe izquierdo

  int primo;
  int bytes = read(pipe_izq[READ], &primo,
                   sizeof(primo)); //se lee el primer numero del pipe izquierdo

  if (bytes == 0) {
    close(
      pipe_izq
        [READ]); //si no hay mas numeros, se cierra la lectura del pipe izquierdo
    exit(0);
    return;
  }
  ...
  if (pid == 0) {          //si es el nieto
    ...
    int numero;
    while (read(pipe_izq[READ], &numero, sizeof(numero)) >
           0) {                  //mientras haya numeros en el pipe izquierdo
      if (numero % primo != 0) { //si el numero no es divisible por el primo
        write(pipe_der[WRITE], &numero,
              sizeof(numero)); //se escribe el numero en el pipe derecho
      }
    }
    close(pipe_izq[READ]); //se cierra la lectura del pipe izquierdo en el padre
    close(
      pipe_der[WRITE]); //se cierra la escritura del pipe derecho en el padre
